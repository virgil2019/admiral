...
func (s *Service) advanceMerged(ctx context.Context, t *store.AdmiralTask, issue *linear.Issue) {
	...
	// A merged leaf sub-issue may have completed its parent "task". Walk
	// one hop up: if every sibling is now completed, enqueue an autonomous
	// verification of the parent. The same helper is the cascade entry-
	// point used by autopilot when an intermediate parent's own verify
	// passes — see internal/cascade.
	cascade.MaybeEnqueueVerify(ctx, s.linear, s.store, s.logger, t.IssueID, t.IssueIdentifier)
}
